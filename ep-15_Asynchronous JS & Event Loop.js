...
// As soon as the 5000ms timer expires the cb(callback) function cannot directly go to Call Stack. Rather cb function is pushed inside the cb queue. Now, the task of the Event Loop is to push whatever is there inside cb queue to Call Stack.
...
// Here, after fetching data from netflix API, the cbF won't go to cb Queue like usual case. but, it goes to "Microtask Queue" {It's given priority over cb Queue}
// So, now cbF goes to Microtask Queue and cbT in cb Queue.
// Microtask Queue has callbacks only from Promises and MutationObserver (The MutationObserver interface provides the ability to watch for changes being made to the DOM tree. )
// jya sudhi microtask queue ma badha callbacks call stack ma na pochadi de tya sudhi Event Loop doesn't put CB Queue call backs to call stack.
